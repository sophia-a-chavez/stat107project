...
cat("Shootings per day in states with no enforced lockdowns",state_avg_shootings_during,"\n",  "Shootings per day in states with no enforced lockdowns",state_avg_shootings_before,"\n", "Shootings per day in  states with no enforced lockdowns",state_avg_shootings_after)
...
# Fatal Shootings Dataset Analytics 
# Compairing Event 1 and 2
```{r}
...
average_age <- c(mean(during_covid$age), mean(shootings_period$age), mean(FatalShootingsStats$age))
...
unarmed_armedp <- c((sum(during_covid$armed == "unarmed") /nrow(during_covid)), (sum(shootings_period$armed == "unarmed") / nrow(shootings_period)), (sum(data_17$armed == "unarmed") / nrow(FatalShootingsStats)))

car_armedp <- c((sum(during_covid$armed == "Vehicle") / nrow(during_covid)), (sum(shootings_period$armed == "Vehicle") / nrow(shootings_period)), (sum(data_17$armed == "Vehicle") / nrow(FatalShootingsStats)))
...
BLM_race_count<- shootings_period %>%
  group_by(race_name) %>%
  summarise(PriceMean = nrow(race_name))
BLM_race_count
...
barplot(race_counts_matrix, beside = TRUE, col = colors,
        ...
        xlab = "Race", ylab = "Number of Fatal Shootings")
...
# creating bar plot
barplot(race_counts_matrix_BLM, beside = TRUE, col = colors,
...
# creating bar plot
barplot(race_counts_matrix_covid, beside = TRUE, col = colors,
